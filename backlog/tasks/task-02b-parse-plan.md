...
- [x] `config/workbook_parse_plan.yaml` exists and is valid YAML.
- [x] Every workbook listed in `docs/inspection_report.md` has corresponding parse-plan entries for each sheet.
- [x] Every sheet is explicitly marked as included, skipped, or split.
- [x] Every included or split sheet has an `output_dataset` and verification target.
- [x] Custom-parser cases are called out explicitly so transform work can be sequenced deliberately.
- [x] No workbook proceeds to transform without a parse-plan entry.

**Status: COMPLETE** — `config/workbook_parse_plan.yaml` generated by `scripts/generate_parse_plan.py`. 230 workbooks, 335 sheets total: 299 included (data), 36 skipped (notes/metadata/unknown). All included sheets have output_dataset and verification_target.
