...
_STDLIB_INSPECT_MODULE = importlib.util.module_from_spec(_STDLIB_INSPECT_SPEC)
_STDLIB_INSPECT_SPEC.loader.exec_module(_STDLIB_INSPECT_MODULE)
sys.modules["inspect"] = _STDLIB_INSPECT_MODULE
...
def _infer_first_data_row(rows: list[list[str]], fiscal_year_columns: list[int]) -> int | None:
    for row_index, row in enumerate(rows, start=1):
        ...
        has_year_data = any(column <= len(row) and row[column - 1] for column in fiscal_year_columns)
        looks_like_header = bool(fiscal_year_columns) and all(
            column <= len(row) and FISCAL_YEAR_RE.match(row[column - 1] or "") for column in fiscal_year_columns
        )
...
def _sheet_has_merged_cells(workbook_path: Path, worksheet_path: str | None) -> bool:
    if not worksheet_path:
        return False
    with zipfile.ZipFile(workbook_path) as archive:
        with archive.open(worksheet_path) as sheet_xml:
            return b"<mergeCell" in sheet_xml.read()
...
def profile_sheet(workbook_path: Path, sheet_name: str) -> dict:
    ...
    first_data_row = _infer_first_data_row(rows, fiscal_year_columns)
    likely_header_rows = _infer_header_rows(first_data_row)
    classification = _classify_sheet(sheet_name, rows, fiscal_year_columns, first_data_row)
    multiple_tables_flagged = _detect_multiple_tables(rows, first_data_row)
    ...
    return {
        ...
        "likely_first_data_row": first_data_row,
...
def render_report(profiles: list[dict]) -> str:
    ...
    lines.append("")
    lines.append("Generated by `src/inspect.py`.")
    lines.append("")
    if not profiles:
