...
from etl.config import PARSE_PLANS_DIR, get_output_path, iter_parse_plan_files
...
def _read_plan(parse_plan_path: Path) -> list[VerificationPlan]:
    ...
    for plan_file in iter_parse_plan_files(parse_plan_path):
            ...
            for sheet_entry in workbook_entry.get("sheets", []):
                plans.append(
                    VerificationPlan(
                        ...
                        header_end_row=transform._header_end_row(
                            str(sheet_entry.get("header_rows", "1-1"))
                        ),
                        ...
                        verification_include_totals=bool(
                            sheet_entry.get("verification_include_totals", False)
                        ),
                        verification_exempt=bool(sheet_entry.get("verification_exempt", False)),
                        verification_exempt_reason=str(
                            sheet_entry.get("verification_exempt_reason", "")
                        ).strip(),
        ...
        try:
            for plan in workbook_plans:
                ...
                if plan.output_dataset not in dataset_cache:
                    ...
                    dataset_cache[plan.output_dataset] = _read_dataset(output_path)
