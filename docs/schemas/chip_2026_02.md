...
| Field | Value |
|---|---|
| Source file(s) | `51296-2026-02-chip.xlsx` |
| Source sheet(s) | `CHIP_02-2026` |
...
| `value` | float | Parsed numeric value from the source cell. | See ``unit`` column | `22.0` | Negative values indicate outflows or reductions. Values originally enclosed in parentheses (e.g. ``(123)``) are converted to negative floats. |
...
| `source_file` | string | Original CBO workbook filename from ``data/raw/``. | N/A | `51296-2026-02-chip.xlsx` | Use this column to trace any row back to its exact source workbook. |
| `source_sheet` | string | Worksheet name within the source workbook. | N/A | `CHIP_02-2026` | Combine with source file, row, and column for exact cell provenance. |
...
| `category_path` | string | Hierarchy-aware path from table/section headings to the leaf category. | N/A | `Actual, 2025 / BUDGET INFORMATION / B...` | Use this field instead of ``category`` when labels repeat in different subprograms. |
...
|---|---|---|---|---|
| Actual, 2025 / BUDGET INFORMATION / Budget Authoritya | `a` | Title XXI of the Social Security Act provides funding through September 2029. Through 2023, budget authority for the program was set by statute. For the 2024-2029 period, budget authority is set to equal the amounts needed to fund allotments. After 2029, consistent with section 257 of the Balanced Budget and Emergency Deficit Control Act of 1985, CBO assumes that the program will continue to operate as it would immediately before its scheduled expiration in 2029. | Budget Authoritya | `51296-2026-02-chip.xlsx` / `CHIP_02-2026` / R13C1 |
| Actual, 2025 / Enrollment at Baseline Funding Amountsb | `b` | Amounts represent the number of beneficiaries enrolled on an average monthly basis in the 50 states and the District of Columbia but exclude enrollment in the U.S. territories. | Enrollment at Baseline Funding Amountsb | `51296-2026-02-chip.xlsx` / `CHIP_02-2026` / R19C1 |
| Actual, 2025 / Enrollment at Baseline Funding Amountsb / Total Enrollment | `b` | Amounts represent the number of beneficiaries enrolled on an average monthly basis in the 50 states and the District of Columbia but exclude enrollment in the U.S. territories. | Enrollment at Baseline Funding Amountsb | `51296-2026-02-chip.xlsx` / `CHIP_02-2026` / R19C1 |
