# Schema: Militaryretirement 2024 06
...
| Field | Value |
|---|---|
| Source file(s) | `51303-2024-06-militaryretirement.xlsx` |
| Source sheet(s) | `Mil Retirement_06-2024` |
| Unit(s) | Dollars per participant, Millions of dollars, Thousands of people |
...
| `value` | float | Parsed numeric value from the source cell. | See ``unit`` column | `78200.0` | Negative values indicate outflows or reductions. Values originally enclosed in parentheses (e.g. ``(123)``) are converted to negative floats. |
...
| `source_file` | string | Original CBO workbook filename from ``data/raw/``. | N/A | `51303-2024-06-militaryretirement.xlsx` | Use this column to trace any row back to its exact source workbook. |
| `source_sheet` | string | Worksheet name within the source workbook. | N/A | `Mil Retirement_06-2024` | Combine with source file, row, and column for exact cell provenance. |
...
|---|---|---|---|---|
| Military Retirement / BUDGET INFORMATION / Adjustment for Payday Shifta | `a` | Section 632 of the Ike Skelton National Defense Authorization Act for Fiscal Year 2011 requires the Department of Defense to make retirement payments on the first day of each month. If the first day of the month occurs on a holiday or a weekend, payments are instead made on the preceding business day. That will result in one additional paycheck in fiscal years 2028 and 2033 and one fewer paycheck in fiscal years 2024 and 2029. Section 632 does not affect the timing of survivor benefit payments. | Adjustment for Payday Shifta | `51303-2024-06-militaryretirement.xlsx` / `Mil Retirement_06-2024` / R17C2 |
