...
| Field | Value |
|---|---|
| Source file(s) | `51313-2024-06-ssi.xlsx` |
| Source sheet(s) | `SSI_06-2024` |
...
| `source_file` | string | Original CBO workbook filename from ``data/raw/``. | N/A | `51313-2024-06-ssi.xlsx` | Use this column to trace any row back to its exact source workbook. |
| `source_sheet` | string | Worksheet name within the source workbook. | N/A | `SSI_06-2024` | Combine with source file, row, and column for exact cell provenance. |
...
|---|---|---|---|---|
| Supplemental Security Income / OTHER INFORMATION / Number of Payments (By fiscal year)a | `a` | SSA is required to make SSI payments on the first day of the month. If that day falls on a holiday or a weekend, SSA will instead make the payments on the preceding business day. As a result, in any given fiscal year there may be 11, 12, or 13 payments. | Number of Payments (By fiscal year)a | `51313-2024-06-ssi.xlsx` / `SSI_06-2024` / R35C1 |
