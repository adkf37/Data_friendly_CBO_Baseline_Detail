...
| `category` | string | Leaf line-item label from the source worksheet. | N/A | `Regular Benefits` | Rows where ``is_total`` is ``true`` represent aggregated totals or subtotals and should be excluded from sum-based aggregations to avoid double-counting. |
...
| `source_sheet` | string | Worksheet name within the source workbook. | N/A | `UI and TAA_01-2025` | Combine with source file, row, and column for exact cell provenance. |
...
|---|---|---|---|---|
| Unemployment Compensation and Trade Adjustment Assistance / Estimated outlays in millions of dollars, by fiscal year / Trade Adjustment Assistancea | `a` | Authorization for the Trade Adjustment Assistance program expired at the end of June 2022. The amounts include cash and training benefits for workers certified before the program expired. | Trade Adjustment Assistancea | `51316-2025-01-unemployment.xlsx` / `UI and TAA_01-2025` / R15C1 |
