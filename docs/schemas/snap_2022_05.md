...
| Field | Value |
|---|---|
| Source file(s) | `51312-2022-05-snap.xlsx` |
| Source sheet(s) | `SNAP_05-2022` |
...
| `value` | float | Parsed numeric value from the source cell. | See ``unit`` column | `150748.0` | Negative values indicate outflows or reductions. Values originally enclosed in parentheses (e.g. ``(123)``) are converted to negative floats. |
...
| `source_file` | string | Original CBO workbook filename from ``data/raw/``. | N/A | `51312-2022-05-snap.xlsx` | Use this column to trace any row back to its exact source workbook. |
| `source_sheet` | string | Worksheet name within the source workbook. | N/A | `SNAP_05-2022` | Combine with source file, row, and column for exact cell provenance. |
...
|---|---|---|---|---|
| Supplemental Nutrition Assistance Program / By fiscal year, millions of dollars / State Administrationa | `a` | Excludes employment and training. | State Administrationa | `51312-2022-05-snap.xlsx` / `SNAP_05-2022` / R47C1 |
