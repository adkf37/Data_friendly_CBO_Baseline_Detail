...
**Vintage:** 2025-01  
**Rows:** 176  
**Fiscal years covered:** 2019–2035  

## Purpose

Tidy long-form CBO baseline data for the **Pellgrant** program(s), extracted from CBO budget baseline workbooks published by the Congressional Budget Office. Each row represents a single program/category/fiscal-year observation.
...
| Unit(s) | Budget authority in millions of dollars, by fiscal year, Millions of dollars, by fiscal year |

## Columns

| Column | Type | Description | Unit | Example | Notes |
|---|---|---|---|---|---|
| `program` | string | CBO program name inferred from the source workbook filename. | N/A | `Pellgrant` | Derived from the workbook filename; may include a version suffix for older files. |
...
| `unit` | string | Unit of measure for the ``value`` column, sourced from the parse plan. | N/A | `Budget authority in millions of dolla...` | Common values include 'Millions of dollars', 'Billions of dollars', and 'Thousands'. |
...
| `source_sheet` | string | Worksheet name within the source workbook. | N/A | `T1_Pell_01-2025` | Combine with ``source_file`` for a fully qualified provenance reference. |
