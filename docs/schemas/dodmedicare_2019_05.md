...
| `value` | float | Parsed numeric value from the source cell. | See ``unit`` column | `2389.0` | Negative values indicate outflows or reductions. Values originally enclosed in parentheses (e.g. ``(123)``) are converted to negative floats. |
...
| `source_file` | string | Original CBO workbook filename from ``data/raw/``. | N/A | `54946-2019-05-DoDMedicare.xlsx` | Use this column to trace any row back to its exact source workbook. |
