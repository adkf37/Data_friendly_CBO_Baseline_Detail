...
| Field | Value |
|---|---|
| Source file(s) | `51299-2020-03-fostercare.xlsx` |
| Source sheet(s) | `Foster Care` |
...
| `value` | float | Parsed numeric value from the source cell. | See ``unit`` column | `1694.0` | Negative values indicate outflows or reductions. Values originally enclosed in parentheses (e.g. ``(123)``) are converted to negative floats. |
...
| `source_file` | string | Original CBO workbook filename from ``data/raw/``. | N/A | `51299-2020-03-fostercare.xlsx` | Use this column to trace any row back to its exact source workbook. |
| `source_sheet` | string | Worksheet name within the source workbook. | N/A | `Foster Care` | Combine with source file, row, and column for exact cell provenance. |
...
| `category_path` | string | Hierarchy-aware path from table/section headings to the leaf category. | N/A | `Outlays in Millions of Dollars, by Fi...` | Use this field instead of ``category`` when labels repeat in different subprograms. |
