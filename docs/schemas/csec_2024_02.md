...
| Field | Value |
|---|---|
| Source file(s) | `51295-2024-02-csec.xlsx` |
| Source sheet(s) | `CSEC_02-2024` |
...
| `value` | float | Parsed numeric value from the source cell. | See ``unit`` column | `3960.0` | Negative values indicate outflows or reductions. Values originally enclosed in parentheses (e.g. ``(123)``) are converted to negative floats. |
...
| `source_file` | string | Original CBO workbook filename from ``data/raw/``. | N/A | `51295-2024-02-csec.xlsx` | Use this column to trace any row back to its exact source workbook. |
| `source_sheet` | string | Worksheet name within the source workbook. | N/A | `CSEC_02-2024` | Combine with source file, row, and column for exact cell provenance. |
...
|---|---|---|---|---|
| Child Support Enforcement and Child Support Collections / Child Support Enforcement Program / Sequestrationa | `a` | The Infrastructure Investment and Jobs Act extended sequestration for mandatory spending through fiscal year 2031. | Sequestrationa | `51295-2024-02-csec.xlsx` / `CSEC_02-2024` / R18C1 |
| Child Support Enforcement and Child Support Collections / Child Support Collectionsb / TANF Collections Retained by the Federal Government | `b` | Collections are recordered as negative outlays. | Child Support Collectionsb | `51295-2024-02-csec.xlsx` / `CSEC_02-2024` / R20C1 |
