# Schema: Fostercare 2023 05

- **Dataset:** `fostercare_2023_05`
- **Vintage:** 2023-05
- **Rows:** 74
- **Fiscal years covered:** 2003–2055

## Purpose

Tidy long-form CBO baseline data for the **Foster Care** program(s), extracted from CBO budget baseline workbooks published by the Congressional Budget Office. Each row represents one numeric source cell with explicit program, category hierarchy, period semantics, and cell-level provenance.
...
| Field | Value |
|---|---|
| Source file(s) | `51299-2023-05-fostercare.xlsx` |
| Source sheet(s) | `Foster Care_05-2023` |
| Unit(s) | Millions of dollars, Thousands of cases |

## Columns

| Column | Type | Description | Unit | Example | Notes |
|---|---|---|---|---|---|
| `program` | string | Canonical CBO program name keyed by the stable source identifier. | N/A | `Foster Care` | Stable across workbook vintages; use ``program_id`` as the machine key. |
...
| `value` | float | Parsed numeric value from the source cell. | See ``unit`` column | `9415.0` | Negative values indicate outflows or reductions. Values originally enclosed in parentheses (e.g. ``(123)``) are converted to negative floats. |
...
| `source_file` | string | Original CBO workbook filename from ``data/raw/``. | N/A | `51299-2023-05-fostercare.xlsx` | Use this column to trace any row back to its exact source workbook. |
| `source_sheet` | string | Worksheet name within the source workbook. | N/A | `Foster Care_05-2023` | Combine with source file, row, and column for exact cell provenance. |
...
| `program_id` | string | Stable numeric CBO identifier from the source filename. | N/A | `51299` | Preferred program join key across vintages. |
| `category_path` | string | Hierarchy-aware path from table/section headings to the leaf category. | N/A | `Foster Care, Adoption Assistance, and...` | Use this field instead of ``category`` when labels repeat in different subprograms. |
...
|---|---|---|---|---|
| Estimated Outlays for Foster Care / Estimated Outlays for Other Components / Sequestrationa | `a` | The Infrastructure Investment and Jobs Act extended sequestration for mandatory spending through fiscal year 2031. | Sequestrationa | `51299-2023-05-fostercare.xlsx` / `Foster Care_05-2023` / R38C1 |
