# Schema: Highwaytrustfund 2024 06
...
| Field | Value |
|---|---|
| Source file(s) | `51300-2024-06-highwaytrustfund.xlsx` |
| Source sheet(s) | `HTF_06-2024` |
...
| `value` | float | Parsed numeric value from the source cell. | See ``unit`` column | `98913.0` | Negative values indicate outflows or reductions. Values originally enclosed in parentheses (e.g. ``(123)``) are converted to negative floats. |
...
| `source_file` | string | Original CBO workbook filename from ``data/raw/``. | N/A | `51300-2024-06-highwaytrustfund.xlsx` | Use this column to trace any row back to its exact source workbook. |
| `source_sheet` | string | Worksheet name within the source workbook. | N/A | `HTF_06-2024` | Combine with source file, row, and column for exact cell provenance. |
...
|---|---|---|---|---|
| Highway Trust Fund Accounts / Highway Account / Flexed Balancesb | `b` | Flexed balances are amounts transferred from the highway account to the transit account. | Flexed Balancesb | `51300-2024-06-highwaytrustfund.xlsx` / `HTF_06-2024` / R15C1 |
| Highway Trust Fund Accounts / Highway Account / Revenues and Interestc | `c` | Some of the taxes that are credited to the Highway Trust Fund are scheduled to expire on September 30, 2028, including the taxes on tires and all but 4.3 cents of the federal tax on motor fuels. However, under the rules governing baseline projections, these estimates reflect the assumption that all of the expiring taxes credited to the fund will continue to be collected after fiscal year 2028. | Revenues and Interestc | `51300-2024-06-highwaytrustfund.xlsx` / `HTF_06-2024` / R16C1 |
| Highway Trust Fund Accounts / Transit Account / Flexed Balancesb | `b` | Flexed balances are amounts transferred from the highway account to the transit account. | Flexed Balancesb | `51300-2024-06-highwaytrustfund.xlsx` / `HTF_06-2024` / R22C1 |
