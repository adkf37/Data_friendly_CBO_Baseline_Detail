...
| Field | Value |
|---|---|
| Source file(s) | `51305-2020-03-pbgc.xlsx` |
| Source sheet(s) | `PBGC` |
...
| `value` | float | Parsed numeric value from the source cell. | See ``unit`` column | `5791.0` | Negative values indicate outflows or reductions. Values originally enclosed in parentheses (e.g. ``(123)``) are converted to negative floats. |
...
| `source_file` | string | Original CBO workbook filename from ``data/raw/``. | N/A | `51305-2020-03-pbgc.xlsx` | Use this column to trace any row back to its exact source workbook. |
| `source_sheet` | string | Worksheet name within the source workbook. | N/A | `PBGC` | Combine with source file, row, and column for exact cell provenance. |
...
|---|---|---|---|---|
| Multiemployer Financial Assistancea | `a` | CBO projects that under current law, the multiemployer revolving fund will be exhausted in 2027. CBO expects that after the fund is exhausted, PBGC will reduce financial assistance to amounts that could be supported with premium income. | Multiemployer Financial Assistancea | `51305-2020-03-pbgc.xlsx` / `PBGC` / R10C1 |
| Premiumsb | `b` | The Bipartisan Budget Act of 2015 accelerated into 2025 the payment of certain premiums that would otherwise be due in 2026. | Premiumsb | `51305-2020-03-pbgc.xlsx` / `PBGC` / R14C1 |
| Other Receiptsc | `c` | "Other receipts" includes reimbursements from the nonbudgetary trust fund to cover a portion of benefits and administrative costs. | Other Receiptsc | `51305-2020-03-pbgc.xlsx` / `PBGC` / R16C1 |
