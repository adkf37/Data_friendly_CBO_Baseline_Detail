...
Tidy long-form CBO baseline data for the **Pellgrant 0** program(s), extracted from CBO budget baseline workbooks published by the Congressional Budget Office. Each row represents a single program/category/fiscal-year observation.
...
|---|---|---|---|---|---|
| `program` | string | CBO program name inferred from the source workbook filename. | N/A | `Pellgrant 0` | Derived from the workbook filename; may include a version suffix for older files. |
...
| `unit` | string | Unit of measure for the ``value`` column, sourced from the parse plan. | N/A | `Historical Pell Grant Funding and Pro...` | Common values include 'Millions of dollars', 'Billions of dollars', and 'Thousands'. |
...
| `source_sheet` | string | Worksheet name within the source workbook. | N/A | `Cumulative Shortfall-Surplus` | Combine with ``source_file`` for a fully qualified provenance reference. |
