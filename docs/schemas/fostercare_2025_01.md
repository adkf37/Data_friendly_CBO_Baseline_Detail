...
| Field | Value |
|---|---|
| Source file(s) | `51299-2025-01-fostercare.xlsx` |
| Source sheet(s) | `Foster Care_01-2025` |
...
| `category` | string | Leaf line-item label from the source worksheet. | N/A | `Maintenance Payments` | Rows where ``is_total`` is ``true`` represent aggregated totals or subtotals and should be excluded from sum-based aggregations to avoid double-counting. |
...
| `value` | float | Parsed numeric value from the source cell. | See ``unit`` column | `1799.0` | Negative values indicate outflows or reductions. Values originally enclosed in parentheses (e.g. ``(123)``) are converted to negative floats. |
...
| `source_file` | string | Original CBO workbook filename from ``data/raw/``. | N/A | `51299-2025-01-fostercare.xlsx` | Use this column to trace any row back to its exact source workbook. |
| `source_sheet` | string | Worksheet name within the source workbook. | N/A | `Foster Care_01-2025` | Combine with source file, row, and column for exact cell provenance. |
...
|---|---|---|---|---|
| Foster Care, Adoption Assistance, and Guardianship / Estimated Outlays for Other Components / Sequestrationa | `a` | The Further Consolidated Appropriations Act, 2024, extended sequestration for mandatory spending through fiscal year 2032. The percentage reduction for that year is smaller than the reduction in place through 2031 | Sequestrationa | `51299-2025-01-fostercare.xlsx` / `Foster Care_01-2025` / R37C1 |
