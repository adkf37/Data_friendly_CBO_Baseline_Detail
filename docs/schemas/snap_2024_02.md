...
| Field | Value |
|---|---|
| Source file(s) | `51312-2024-02-snap.xlsx` |
| Source sheet(s) | `SNAP_02-2024-rev` |
...
| `value` | float | Parsed numeric value from the source cell. | See ``unit`` column | `110786.0` | Negative values indicate outflows or reductions. Values originally enclosed in parentheses (e.g. ``(123)``) are converted to negative floats. |
...
| `source_file` | string | Original CBO workbook filename from ``data/raw/``. | N/A | `51312-2024-02-snap.xlsx` | Use this column to trace any row back to its exact source workbook. |
| `source_sheet` | string | Worksheet name within the source workbook. | N/A | `SNAP_02-2024-rev` | Combine with source file, row, and column for exact cell provenance. |
...
|---|---|---|---|---|
| Supplemental Nutrition Assistance Program / Thrifty Food Plan, Estimated Change / June/June Preceding Year, Lagged (Percent)a* | `a` | CBO increased its projections of the cost of the Thrifty Food Plan (TFP) in 2027 and 2032, anticipating that reevaluations of the TFP, which occur every five years, would increase costs. | June/June Preceding Year, Lagged (Percent)a* | `51312-2024-02-snap.xlsx` / `SNAP_02-2024-rev` / R37C1 |
| Supplemental Nutrition Assistance Program / BUDGET AUTHORITY OTHER THAN BENEFITS AND NUTRITION ASSISTANCE FOR PUERTO RICO AND AMERICAN SAMOA / State Administrationb | `b` | Excludes employment and training. * On February 22, 2024, CBO updated numbers in the row labeled “Thrifty Food Plan, Estimated Change, June/June Preceding Year, Lagged (Percent).” | State Administrationb | `51312-2024-02-snap.xlsx` / `SNAP_02-2024-rev` / R44C1 |
