...
Tidy long-form CBO baseline data for the **Usda** program(s), extracted from CBO budget baseline workbooks published by the Congressional Budget Office. Each row represents a single program/category/fiscal-year observation.
...
| Source sheet(s) | `AppA_USDA Baseline_01-2025`, `AppB_USDA Baseline_01-2025` |
| Unit(s) | By Fiscal Year, Millions of Dollars, Under the $30 Billion Borrowing Limit |

## Columns

| Column | Type | Description | Unit | Example | Notes |
|---|---|---|---|---|---|
| `program` | string | CBO program name inferred from the source workbook filename. | N/A | `Usda` | Derived from the workbook filename; may include a version suffix for older files. |
...
| `value` | float | Parsed numeric value from the source cell. | See ``unit`` column | `-186.0` | Negative values indicate outflows or reductions. Values originally enclosed in parentheses (e.g. ``(123)``) are converted to negative floats. |
...
| `source_sheet` | string | Worksheet name within the source workbook. | N/A | `AppA_USDA Baseline_01-2025` | Combine with ``source_file`` for a fully qualified provenance reference. |
