...
| Field | Value |
|---|---|
| Source file(s) | `51295-2025-01-csec.xlsx` |
| Source sheet(s) | `CSEC_01-2025` |
...
| `value` | float | Parsed numeric value from the source cell. | See ``unit`` column | `4024.0` | Negative values indicate outflows or reductions. Values originally enclosed in parentheses (e.g. ``(123)``) are converted to negative floats. |
...
| `source_file` | string | Original CBO workbook filename from ``data/raw/``. | N/A | `51295-2025-01-csec.xlsx` | Use this column to trace any row back to its exact source workbook. |
| `source_sheet` | string | Worksheet name within the source workbook. | N/A | `CSEC_01-2025` | Combine with source file, row, and column for exact cell provenance. |
...
|---|---|---|---|---|
| Child Support Enforcement and Child Support Collections / Child Support Enforcement Program / Sequestrationa | `a` | The Further Consolidated Appropriations Act, 2024, extended sequestration for mandatory spending through fiscal year 2032. The percentage reduction for that year is smaller than the reduction in place through 2031. | Sequestrationa | `51295-2025-01-csec.xlsx` / `CSEC_01-2025` / R18C1 |
| Child Support Enforcement and Child Support Collections / Child Support Collectionsb / TANF Collections Retained by the Federal Government | `b` | Collections are recorded as negative outlays. | Child Support Collectionsb | `51295-2025-01-csec.xlsx` / `CSEC_01-2025` / R22C1 |
