# Schema: Healthinsurance 2022 06
...
| Field | Value |
|---|---|
| Source file(s) | `51298-2022-06-healthinsurance.xlsx` |
...
| `value` | float | Parsed numeric value from the source cell. | See ``unit`` column | `270.6` | Negative values indicate outflows or reductions. Values originally enclosed in parentheses (e.g. ``(123)``) are converted to negative floats. |
...
| `source_file` | string | Original CBO workbook filename from ``data/raw/``. | N/A | `51298-2022-06-healthinsurance.xlsx` | Use this column to trace any row back to its exact source workbook. |
...
| `category_path` | string | Hierarchy-aware path from table/section headings to the leaf category. | N/A | `Table 1. Health Insurance Coverage fo...` | Use this field instead of ``category`` when labels repeat in different subprograms. |
