# Schema: Childsupportenforcement 2020 01
...
| Field | Value |
|---|---|
| Source file(s) | `51295-2020-01-childsupportenforcement.xlsx` |
| Source sheet(s) | `Child Support Collections` |
...
| `value` | float | Parsed numeric value from the source cell. | See ``unit`` column | `3539.0` | Negative values indicate outflows or reductions. Values originally enclosed in parentheses (e.g. ``(123)``) are converted to negative floats. |
...
| `source_file` | string | Original CBO workbook filename from ``data/raw/``. | N/A | `51295-2020-01-childsupportenforcement...` | Use this column to trace any row back to its exact source workbook. |
| `source_sheet` | string | Worksheet name within the source workbook. | N/A | `Child Support Collections` | Combine with source file, row, and column for exact cell provenance. |
...
| `category_path` | string | Hierarchy-aware path from table/section headings to the leaf category. | N/A | `Child Support Enforcement / Child Sup...` | Use this field instead of ``category`` when labels repeat in different subprograms. |
...
| `source_row` | integer | One-based worksheet row containing the numeric source value. | N/A | `5` | Together with ``source_column`` identifies the exact source cell. |
