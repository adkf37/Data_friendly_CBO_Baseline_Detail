...
| Field | Value |
|---|---|
| Source file(s) | `51313-2021-02-ssi.xlsx` |
| Source sheet(s) | `SSI_2-2021` |
| Unit(s) | Billions of dollars, Dollars, Dollars per beneficiary, Number of paymentsa, Percentage, Thousands of people |
...
| `value` | float | Parsed numeric value from the source cell. | See ``unit`` column | `57.0` | Negative values indicate outflows or reductions. Values originally enclosed in parentheses (e.g. ``(123)``) are converted to negative floats. |
...
| `source_file` | string | Original CBO workbook filename from ``data/raw/``. | N/A | `51313-2021-02-ssi.xlsx` | Use this column to trace any row back to its exact source workbook. |
| `source_sheet` | string | Worksheet name within the source workbook. | N/A | `SSI_2-2021` | Combine with source file, row, and column for exact cell provenance. |
...
|---|---|---|---|---|
| Supplemental Security Income / Unlabeled value (row 44) | `a` | The Social Security Administration (SSA) is required to pay Supplemental Security Income payments on the first day of the month. If the first day of the month occurs on a holiday or a weekend, SSA will instead make the payments on the preceding business day. As a result, in any given fiscal year there may be 11, 12, or 13 payments. | Number of Paymentsa | `51313-2021-02-ssi.xlsx` / `SSI_2-2021` / R44C2 |
