...
| Field | Value |
|---|---|
| Source file(s) | `51317-2023-05-usda.xlsx` |
| Source sheet(s) | `App 117-169 CCC funding`, `USDA_05-2023` |
...
| `value` | float | Parsed numeric value from the source cell. | See ``unit`` column | `4821.0` | Negative values indicate outflows or reductions. Values originally enclosed in parentheses (e.g. ``(123)``) are converted to negative floats. |
...
| `source_file` | string | Original CBO workbook filename from ``data/raw/``. | N/A | `51317-2023-05-usda.xlsx` | Use this column to trace any row back to its exact source workbook. |
| `source_sheet` | string | Worksheet name within the source workbook. | N/A | `USDA_05-2023` | Combine with source file, row, and column for exact cell provenance. |
...
| `source_row` | integer | One-based worksheet row containing the numeric source value. | N/A | `114` | Together with ``source_column`` identifies the exact source cell. |
