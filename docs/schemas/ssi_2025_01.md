...
| Field | Value |
|---|---|
| Source file(s) | `51313-2025-01-ssi.xlsx` |
| Source sheet(s) | `SSI_01-2025` |
...
| `value` | float | Parsed numeric value from the source cell. | See ``unit`` column | `64.9` | Negative values indicate outflows or reductions. Values originally enclosed in parentheses (e.g. ``(123)``) are converted to negative floats. |
...
| `source_file` | string | Original CBO workbook filename from ``data/raw/``. | N/A | `51313-2025-01-ssi.xlsx` | Use this column to trace any row back to its exact source workbook. |
| `source_sheet` | string | Worksheet name within the source workbook. | N/A | `SSI_01-2025` | Combine with source file, row, and column for exact cell provenance. |
