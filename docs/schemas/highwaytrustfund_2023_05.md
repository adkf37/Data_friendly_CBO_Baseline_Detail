# Schema: Highwaytrustfund 2023 05
...
| Field | Value |
|---|---|
| Source file(s) | `51300-2023-05-highwaytrustfund.xlsx` |
| Source sheet(s) | `HTF_05-2023` |
...
| `value` | float | Parsed numeric value from the source cell. | See ``unit`` column | `14264.0` | Negative values indicate outflows or reductions. Values originally enclosed in parentheses (e.g. ``(123)``) are converted to negative floats. |
...
| `source_file` | string | Original CBO workbook filename from ``data/raw/``. | N/A | `51300-2023-05-highwaytrustfund.xlsx` | Use this column to trace any row back to its exact source workbook. |
| `source_sheet` | string | Worksheet name within the source workbook. | N/A | `HTF_05-2023` | Combine with source file, row, and column for exact cell provenance. |
...
|---|---|---|---|---|
| Highway Trust Fund Accounts / Highway Account / Flexed Balancesb | `b` | Flexed balances are amounts transferred from the highway account to the transit account. | Flexed Balancesb | `51300-2023-05-highwaytrustfund.xlsx` / `HTF_05-2023` / R15C1 |
| Highway Trust Fund Accounts / Highway Account / Revenues and Interestc | `c` | Some of the taxes that are credited to the Highway Trust Fund are scheduled to expire on September 30, 2028, including the taxes on tires and all but 4.3 cents of the federal tax on motor fuels. However, under the rules governing baseline projections, these estimates reflect the assumption that all of the expiring taxes credited to the fund will continue to be collected after fiscal year 2028. | Revenues and Interestc | `51300-2023-05-highwaytrustfund.xlsx` / `HTF_05-2023` / R16C1 |
| Highway Trust Fund Accounts / Highway Account / Intragovernmental Transfersd | `d` | In 2022, section 80103 of the Infrastructure Investments and Jobs Act required intragovernmental transfers of a combined $118 billion from the general fund of the Treasury to the Highway Trust Fund. | Intragovernmental Transfersd | `51300-2023-05-highwaytrustfund.xlsx` / `HTF_05-2023` / R17C1 |
| Highway Trust Fund Accounts / Transit Account / Flexed Balancesb | `b` | Flexed balances are amounts transferred from the highway account to the transit account. | Flexed Balancesb | `51300-2023-05-highwaytrustfund.xlsx` / `HTF_05-2023` / R23C1 |
| Highway Trust Fund Accounts / Transit Account / Revenues and Interestc | `c` | Some of the taxes that are credited to the Highway Trust Fund are scheduled to expire on September 30, 2028, including the taxes on tires and all but 4.3 cents of the federal tax on motor fuels. However, under the rules governing baseline projections, these estimates reflect the assumption that all of the expiring taxes credited to the fund will continue to be collected after fiscal year 2028. | Revenues and Interestc | `51300-2023-05-highwaytrustfund.xlsx` / `HTF_05-2023` / R24C1 |
| Highway Trust Fund Accounts / Transit Account / Intragovernmental Transfersd | `d` | In 2022, section 80103 of the Infrastructure Investments and Jobs Act required intragovernmental transfers of a combined $118 billion from the general fund of the Treasury to the Highway Trust Fund. | Intragovernmental Transfersd | `51300-2023-05-highwaytrustfund.xlsx` / `HTF_05-2023` / R25C1 |
| Highway Trust Fund Accounts / Cumulative Shortfalla / Highway Account | `a` | Under current law, the Highway Trust Fund cannot incur negative balances. However, following the rules governing baseline projections in the Balanced Budget and Emergency Deficit Control Act of 1985, CBO’s baseline for surface transportation spending reflects the assumption that obligations presented to the Highway Trust Fund will be paid in full. The memorandum to this table shows the cumulative shortfall of fund balances, assuming spending amounts consistent with CBO’s May 2023 baseline. Following the rules for baseline construction, those amounts are estimated by adjusting the obligation limitations enacted under the Consolidated Appropriations Act, 2023, by projected inflation. | Cumulative Shortfalla | `51300-2023-05-highwaytrustfund.xlsx` / `HTF_05-2023` / R31C1 |
