...
| Field | Value |
|---|---|
| Source file(s) | `51312-2020-03-snap.xlsx` |
| Source sheet(s) | `SNAP` |
...
| `value` | float | Parsed numeric value from the source cell. | See ``unit`` column | `61853.0` | Negative values indicate outflows or reductions. Values originally enclosed in parentheses (e.g. ``(123)``) are converted to negative floats. |
...
| `source_file` | string | Original CBO workbook filename from ``data/raw/``. | N/A | `51312-2020-03-snap.xlsx` | Use this column to trace any row back to its exact source workbook. |
...
|---|---|---|---|---|
| Fiscal Year Average / Budget Authority Other Than Benefits and Nutrition Assistance for Puerto Rico and American Samoa / State Administrationa | `a` | Excludes employment and training. | State Administrationa | `51312-2020-03-snap.xlsx` / `SNAP` / R42C1 |
