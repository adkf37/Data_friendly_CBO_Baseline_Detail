...
| Field | Value |
|---|---|
| Source file(s) | `51309-2020-03-trustfund.xlsx` |
| Source sheet(s) | `Social Security Trust Funds` |
...
| `category` | string | Leaf line-item label from the source worksheet. | N/A | `Payroll Tax Revenues` | Rows where ``is_total`` is ``true`` represent aggregated totals or subtotals and should be excluded from sum-based aggregations to avoid double-counting. |
...
| `value` | float | Parsed numeric value from the source cell. | See ``unit`` column | `914.0` | Negative values indicate outflows or reductions. Values originally enclosed in parentheses (e.g. ``(123)``) are converted to negative floats. |
...
| `source_file` | string | Original CBO workbook filename from ``data/raw/``. | N/A | `51309-2020-03-trustfund.xlsx` | Use this column to trace any row back to its exact source workbook. |
| `source_sheet` | string | Worksheet name within the source workbook. | N/A | `Social Security Trust Funds` | Combine with source file, row, and column for exact cell provenance. |
...
| `category_path` | string | Hierarchy-aware path from table/section headings to the leaf category. | N/A | `Payroll Tax Revenues` | Use this field instead of ``category`` when labels repeat in different subprograms. |
...
|---|---|---|---|---|
| Other Income / Federal payroll tax, employer sharea | `a` | Transfers from the general fund to the Social Security trust funds. Such transfers appear in the budget both as positive outlays (the general fund portion) and as negative outlays, or as offsetting receipts (the trust fund portion). | Federal payroll tax, employer sharea | `51309-2020-03-trustfund.xlsx` / `Social Security Trust Funds` / R12C1 |
| Other Income / Interesta | `a` | Transfers from the general fund to the Social Security trust funds. Such transfers appear in the budget both as positive outlays (the general fund portion) and as negative outlays, or as offsetting receipts (the trust fund portion). | Interesta | `51309-2020-03-trustfund.xlsx` / `Social Security Trust Funds` / R13C1 |
