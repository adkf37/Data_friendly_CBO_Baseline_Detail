...
| Field | Value |
|---|---|
| Source file(s) | `51312-2019-05-SNAP.xlsx` |
| Source sheet(s) | `Table 1` |
| Unit(s) | Dollars per person, Millions of dollars, Millions of people, Percent |
...
| `value` | float | Parsed numeric value from the source cell. | See ``unit`` column | `63668.0` | Negative values indicate outflows or reductions. Values originally enclosed in parentheses (e.g. ``(123)``) are converted to negative floats. |
...
| `source_file` | string | Original CBO workbook filename from ``data/raw/``. | N/A | `51312-2019-05-SNAP.xlsx` | Use this column to trace any row back to its exact source workbook. |
...
| `category_path` | string | Hierarchy-aware path from table/section headings to the leaf category. | N/A | `Baseline Totals / Budget Authority` | Use this field instead of ``category`` when labels repeat in different subprograms. |
...
|---|---|---|---|---|
| Fiscal Year Average / Budget Authority Other Than Benefits and Nutrition Assistance for Puerto Rico and American Samoa / State Administrationa | `a` | Excludes employment and training. | State Administrationa | `51312-2019-05-SNAP.xlsx` / `Table 1` / R42C1 |
