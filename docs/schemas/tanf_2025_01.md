...
| Field | Value |
|---|---|
| Source file(s) | `51314-2025-01-tanf.xlsx` |
| Source sheet(s) | `TANF_01-2025` |
...
| `value` | float | Parsed numeric value from the source cell. | See ``unit`` column | `16739.0` | Negative values indicate outflows or reductions. Values originally enclosed in parentheses (e.g. ``(123)``) are converted to negative floats. |
...
| `source_file` | string | Original CBO workbook filename from ``data/raw/``. | N/A | `51314-2025-01-tanf.xlsx` | Use this column to trace any row back to its exact source workbook. |
| `source_sheet` | string | Worksheet name within the source workbook. | N/A | `TANF_01-2025` | Combine with source file, row, and column for exact cell provenance. |
...
| `category_path` | string | Hierarchy-aware path from table/section headings to the leaf category. | N/A | `Temporary Assistance for Needy Famili...` | Use this field instead of ``category`` when labels repeat in different subprograms. |
...
|---|---|---|---|---|
| Temporary Assistance for Needy Families / Budget Authority for TANF Components / Sequestrationa | `a` | The Further Consolidated Appropriations Act, 2024, extended sequestration for mandatory spending through fiscal year 2032. The percentage reduction for that year is smaller than the reduction in place through 2031. | Sequestrationa | `51314-2025-01-tanf.xlsx` / `TANF_01-2025` / R23C1 |
