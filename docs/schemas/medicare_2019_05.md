...
| Field | Value |
|---|---|
| Source file(s) | `51302-2019-05-Medicare.xlsx` |
| Source sheet(s) | `Table 1` |
| Unit(s) | Billions of dollars, Percent |
...
| `category` | string | Leaf line-item label from the source worksheet. | N/A | `Mandatory Outlays (a)` | Rows where ``is_total`` is ``true`` represent aggregated totals or subtotals and should be excluded from sum-based aggregations to avoid double-counting. |
...
| `value` | float | Parsed numeric value from the source cell. | See ``unit`` column | `705.0` | Negative values indicate outflows or reductions. Values originally enclosed in parentheses (e.g. ``(123)``) are converted to negative floats. |
...
| `source_file` | string | Original CBO workbook filename from ``data/raw/``. | N/A | `51302-2019-05-Medicare.xlsx` | Use this column to trace any row back to its exact source workbook. |
...
| `category_path` | string | Hierarchy-aware path from table/section headings to the leaf category. | N/A | `Medicare Totals (Billions of dollars)...` | Use this field instead of ``category`` when labels repeat in different subprograms. |
...
No superscript variable notes are attached to this dataset.
