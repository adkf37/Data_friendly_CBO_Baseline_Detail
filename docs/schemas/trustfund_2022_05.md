...
| Field | Value |
|---|---|
| Source file(s) | `51309-2022-05-trustfund.xlsx` |
| Source sheet(s) | `SS Trustfunds_05-2022` |
...
| `value` | float | Parsed numeric value from the source cell. | See ``unit`` column | `952.0` | Negative values indicate outflows or reductions. Values originally enclosed in parentheses (e.g. ``(123)``) are converted to negative floats. |
...
| `source_file` | string | Original CBO workbook filename from ``data/raw/``. | N/A | `51309-2022-05-trustfund.xlsx` | Use this column to trace any row back to its exact source workbook. |
| `source_sheet` | string | Worksheet name within the source workbook. | N/A | `SS Trustfunds_05-2022` | Combine with source file, row, and column for exact cell provenance. |
...
|---|---|---|---|---|
| Social Security Trust Funds / Other Income / Federal payroll tax, employer sharea | `a` | Transfers from the general fund to the Social Security trust funds. Such transfers appear in the budget both as positive outlays (the general fund portion) and as negative outlays (offsetting receipts; the trust fund portion). | Federal payroll tax, employer sharea | `51309-2022-05-trustfund.xlsx` / `SS Trustfunds_05-2022` / R17C2 |
| Social Security Trust Funds / Other Income / Interesta | `a` | Transfers from the general fund to the Social Security trust funds. Such transfers appear in the budget both as positive outlays (the general fund portion) and as negative outlays (offsetting receipts; the trust fund portion). | Interesta | `51309-2022-05-trustfund.xlsx` / `SS Trustfunds_05-2022` / R18C2 |
