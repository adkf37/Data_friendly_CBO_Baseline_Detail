...
Tidy long-form CBO baseline data for the **Customs Fees** program(s), extracted from CBO budget baseline workbooks published by the Congressional Budget Office. Each row represents one numeric source cell with explicit program, category hierarchy, period semantics, and cell-level provenance.

## Provenance

| Field | Value |
|---|---|
| Source file(s) | `61170-2025-01-customs-fees.xlsx` |
| Source sheet(s) | `Customs Fees Baseline 01-2025` |
...
|---|---|---|---|---|---|
| `program` | string | Canonical CBO program name keyed by the stable source identifier. | N/A | `Customs Fees` | Stable across workbook vintages; use ``program_id`` as the machine key. |
| `category` | string | Leaf line-item label from the source worksheet. | N/A | `Estmated Outlays` | Rows where ``is_total`` is ``true`` represent aggregated totals or subtotals and should be excluded from sum-based aggregations to avoid double-counting. |
...
| `value` | float | Parsed numeric value from the source cell. | See ``unit`` column | `0.0` | Negative values indicate outflows or reductions. Values originally enclosed in parentheses (e.g. ``(123)``) are converted to negative floats. |
...
| `source_file` | string | Original CBO workbook filename from ``data/raw/``. | N/A | `61170-2025-01-customs-fees.xlsx` | Use this column to trace any row back to its exact source workbook. |
| `source_sheet` | string | Worksheet name within the source workbook. | N/A | `Customs Fees Baseline 01-2025` | Combine with source file, row, and column for exact cell provenance. |
...
| `program_id` | string | Stable numeric CBO identifier from the source filename. | N/A | `61170` | Preferred program join key across vintages. |
| `category_path` | string | Hierarchy-aware path from table/section headings to the leaf category. | N/A | `Customs User Fees / One-Year Extensio...` | Use this field instead of ``category`` when labels repeat in different subprograms. |
