# Schema: Child Support Enforcement 2019 05
...
| Field | Value |
|---|---|
| Source file(s) | `51295-2019-05-Child-Support-Enforcement.xlsx` |
| Source sheet(s) | `Sheet3` |
...
| `value` | float | Parsed numeric value from the source cell. | See ``unit`` column | `3513.0` | Negative values indicate outflows or reductions. Values originally enclosed in parentheses (e.g. ``(123)``) are converted to negative floats. |
...
| `source_file` | string | Original CBO workbook filename from ``data/raw/``. | N/A | `51295-2019-05-Child-Support-Enforceme...` | Use this column to trace any row back to its exact source workbook. |
| `source_sheet` | string | Worksheet name within the source workbook. | N/A | `Sheet3` | Combine with source file, row, and column for exact cell provenance. |
...
| `category_path` | string | Hierarchy-aware path from table/section headings to the leaf category. | N/A | `Child Support Administration` | Use this field instead of ``category`` when labels repeat in different subprograms. |
