...
Tidy long-form CBO baseline data for the **Child Support Enforcement** program(s), extracted from CBO budget baseline workbooks published by the Congressional Budget Office. Each row represents a single program/category/fiscal-year observation.
...
|---|---|---|---|---|---|
| `program` | string | CBO program name inferred from the source workbook filename. | N/A | `Child Support Enforcement` | Derived from the workbook filename; may include a version suffix for older files. |
...
| `source_sheet` | string | Worksheet name within the source workbook. | N/A | `Sheet3` | Combine with ``source_file`` for a fully qualified provenance reference. |
