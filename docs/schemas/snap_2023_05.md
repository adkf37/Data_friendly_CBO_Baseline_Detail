...
| Field | Value |
|---|---|
| Source file(s) | `51312-2023-05-snap.xlsx` |
| Source sheet(s) | `SNAP_05-2023` |
...
| `value` | float | Parsed numeric value from the source cell. | See ``unit`` column | `129389.0` | Negative values indicate outflows or reductions. Values originally enclosed in parentheses (e.g. ``(123)``) are converted to negative floats. |
...
| `source_file` | string | Original CBO workbook filename from ``data/raw/``. | N/A | `51312-2023-05-snap.xlsx` | Use this column to trace any row back to its exact source workbook. |
| `source_sheet` | string | Worksheet name within the source workbook. | N/A | `SNAP_05-2023` | Combine with source file, row, and column for exact cell provenance. |
...
|---|---|---|---|---|
| Supplemental Nutrition Assistance Program / Thrifty Food Plan, Estimated Change / June/June Preceding Year, Lagged (Percent)a | `a` | CBO increased its projections of the cost of the Thrifty Food Plan (TFP) in 2027 and 2032, anticipating that reevaluations of the TFP, which occur every five years, would increase costs. | June/June Preceding Year, Lagged (Percent)a | `51312-2023-05-snap.xlsx` / `SNAP_05-2023` / R39C1 |
| Supplemental Nutrition Assistance Program / By fiscal year, millions of dollars / State Administrationb | `b` | Excludes employment and training. | State Administrationb | `51312-2023-05-snap.xlsx` / `SNAP_05-2023` / R46C1 |
