...
| Field | Value |
|---|---|
| Source file(s) | `51293-2020-01-childnutrition.xlsx` |
| Source sheet(s) | `CNP` |
...
| `value` | float | Parsed numeric value from the source cell. | See ``unit`` column | `24148.0` | Negative values indicate outflows or reductions. Values originally enclosed in parentheses (e.g. ``(123)``) are converted to negative floats. |
...
| `source_file` | string | Original CBO workbook filename from ``data/raw/``. | N/A | `51293-2020-01-childnutrition.xlsx` | Use this column to trace any row back to its exact source workbook. |
| `source_sheet` | string | Worksheet name within the source workbook. | N/A | `CNP` | Combine with source file, row, and column for exact cell provenance. |
...
| `category_path` | string | Hierarchy-aware path from table/section headings to the leaf category. | N/A | `Budget Authority` | Use this field instead of ``category`` when labels repeat in different subprograms. |
...
| `source_row` | integer | One-based worksheet row containing the numeric source value. | N/A | `6` | Together with ``source_column`` identifies the exact source cell. |
