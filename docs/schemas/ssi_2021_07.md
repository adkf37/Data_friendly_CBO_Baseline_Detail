...
| Field | Value |
|---|---|
| Source file(s) | `51313-2021-07-ssi.xlsx` |
| Source sheet(s) | `SSI_07-2021` |
| Unit(s) | Billions of dollars, Dollars per beneficiary, Number of paymentsa, Percentage, Thousands of people |
...
| `value` | float | Parsed numeric value from the source cell. | See ``unit`` column | `56.0` | Negative values indicate outflows or reductions. Values originally enclosed in parentheses (e.g. ``(123)``) are converted to negative floats. |
...
| `source_file` | string | Original CBO workbook filename from ``data/raw/``. | N/A | `51313-2021-07-ssi.xlsx` | Use this column to trace any row back to its exact source workbook. |
| `source_sheet` | string | Worksheet name within the source workbook. | N/A | `SSI_07-2021` | Combine with source file, row, and column for exact cell provenance. |
...
| `category_path` | string | Hierarchy-aware path from table/section headings to the leaf category. | N/A | `Supplemental Security Income / Outlay...` | Use this field instead of ``category`` when labels repeat in different subprograms. |
...
|---|---|---|---|---|
| Supplemental Security Income / Unlabeled value (row 38) | `a` | The Social Security Administration (SSA) is required to pay Supplemental Security Income payments on the first day of the month. If the first day of the month occurs on a holiday or a weekend, SSA will instead make the payments on the preceding business day. As a result, in any given fiscal year there may be 11, 12, or 13 payments. | Number of Paymentsa | `51313-2021-07-ssi.xlsx` / `SSI_07-2021` / R38C2 |
