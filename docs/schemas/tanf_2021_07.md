...
| Field | Value |
|---|---|
| Source file(s) | `51314-2021-07-tanf.xlsx` |
| Source sheet(s) | `TANF_` |
...
| `value` | float | Parsed numeric value from the source cell. | See ``unit`` column | `17736.0` | Negative values indicate outflows or reductions. Values originally enclosed in parentheses (e.g. ``(123)``) are converted to negative floats. |
...
| `source_file` | string | Original CBO workbook filename from ``data/raw/``. | N/A | `51314-2021-07-tanf.xlsx` | Use this column to trace any row back to its exact source workbook. |
| `source_sheet` | string | Worksheet name within the source workbook. | N/A | `TANF_` | Combine with source file, row, and column for exact cell provenance. |
...
|---|---|---|---|---|
| Temporary Assistance for Needy Families / Estimated Budget Authority for TANF Components / Sequestrationa | `a` | The Coronavirus Aid, Relief, and Economic Security Act extended sequestration for mandatory spending through fiscal year 2030. | Sequestrationa | `51314-2021-07-tanf.xlsx` / `TANF_` / R25C2 |
