...
Tidy long-form CBO baseline data for the **Health Insurance** program(s), extracted from CBO budget baseline workbooks published by the Congressional Budget Office. Each row represents a single program/category/fiscal-year observation.
...
| Source sheet(s) | `Table 1`, `Table 1-1`, `Table 2`, `Table 2-1`, `Table 3`, `Table 3-1` |
| Unit(s) | (Millions of people)a, Billions of Dollars, by Fiscal Year, Insurance Coverage for the Yeara (Millions of people), Millions of People, by Calendar Year |

## Columns

| Column | Type | Description | Unit | Example | Notes |
|---|---|---|---|---|---|
| `program` | string | CBO program name inferred from the source workbook filename. | N/A | `Health Insurance` | Derived from the workbook filename; may include a version suffix for older files. |
