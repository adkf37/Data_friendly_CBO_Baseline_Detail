...
| Field | Value |
|---|---|
| Source file(s) | `51312-2026-02-snap.xlsx` |
| Source sheet(s) | `SNAP_02-2026` |
| Unit(s) | Billions of dollars, Dollars per participant, Millions of dollars, Millions of people, Percent |
...
| `value` | float | Parsed numeric value from the source cell. | See ``unit`` column | `98415.0` | Negative values indicate outflows or reductions. Values originally enclosed in parentheses (e.g. ``(123)``) are converted to negative floats. |
...
| `source_file` | string | Original CBO workbook filename from ``data/raw/``. | N/A | `51312-2026-02-snap.xlsx` | Use this column to trace any row back to its exact source workbook. |
| `source_sheet` | string | Worksheet name within the source workbook. | N/A | `SNAP_02-2026` | Combine with source file, row, and column for exact cell provenance. |
...
|---|---|---|---|---|
| Supplemental Nutrition Assistance Program / NUTRITION ASSISTANCE FOR PUERTO RICO AND AMERICAN SAMOA / State Administrationa | `a` | Excludes employment and training. | State Administrationa | `51312-2026-02-snap.xlsx` / `SNAP_02-2026` / R45C1 |
