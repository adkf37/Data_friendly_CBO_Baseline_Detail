...
| Unit(s) | Billions of dollars, by fiscal year, Millions of people, by calendar year |
...
| `category` | string | Line-item label as it appears in the source worksheet after header normalization. | N/A | `Total population` | Rows where ``is_total`` is ``true`` represent aggregated totals or subtotals and should be excluded from sum-based aggregations to avoid double-counting. |
...
| `unit` | string | Unit of measure for the ``value`` column, sourced from the parse plan. | N/A | `Millions of people, by calendar year` | Common values include 'Millions of dollars', 'Billions of dollars', and 'Thousands'. |
