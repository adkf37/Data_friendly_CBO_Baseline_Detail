...
| `category` | string | Line-item label as it appears in the source worksheet after header normalization. | N/A | `Single-Employer Benefit Payments` | Rows where ``is_total`` is ``true`` represent aggregated totals or subtotals and should be excluded from sum-based aggregations to avoid double-counting. |
...
| `source_sheet` | string | Worksheet name within the source workbook. | N/A | `PBGC` | Combine with ``source_file`` for a fully qualified provenance reference. |
