...
| Field | Value |
|---|---|
| Source file(s) | `51299-2020-01-fostercare.xlsx` |
...
| `value` | float | Parsed numeric value from the source cell. | See ``unit`` column | `1713.0` | Negative values indicate outflows or reductions. Values originally enclosed in parentheses (e.g. ``(123)``) are converted to negative floats. |
...
| `source_file` | string | Original CBO workbook filename from ``data/raw/``. | N/A | `51299-2020-01-fostercare.xlsx` | Use this column to trace any row back to its exact source workbook. |
...
| `source_row` | integer | One-based worksheet row containing the numeric source value. | N/A | `4` | Together with ``source_column`` identifies the exact source cell. |
