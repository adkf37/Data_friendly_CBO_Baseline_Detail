...
Tidy long-form CBO baseline data for the **Fdic** program(s), extracted from CBO budget baseline workbooks published by the Congressional Budget Office. Each row represents a single program/category/fiscal-year observation.
...
|---|---|---|---|---|---|
| `program` | string | CBO program name inferred from the source workbook filename. | N/A | `Fdic` | Derived from the workbook filename; may include a version suffix for older files. |
...
| `source_sheet` | string | Worksheet name within the source workbook. | N/A | `FDIC_06-2024` | Combine with ``source_file`` for a fully qualified provenance reference. |
