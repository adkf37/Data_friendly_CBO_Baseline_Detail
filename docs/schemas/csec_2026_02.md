...
| Field | Value |
|---|---|
| Source file(s) | `51295-2026-02-csec.xlsx` |
| Source sheet(s) | `CSEC_02-2026` |
...
| `category` | string | Leaf line-item label from the source worksheet. | N/A | `Child Support Administrationa` | Rows where ``is_total`` is ``true`` represent aggregated totals or subtotals and should be excluded from sum-based aggregations to avoid double-counting. |
...
| `value` | float | Parsed numeric value from the source cell. | See ``unit`` column | `4386.0` | Negative values indicate outflows or reductions. Values originally enclosed in parentheses (e.g. ``(123)``) are converted to negative floats. |
...
| `source_file` | string | Original CBO workbook filename from ``data/raw/``. | N/A | `51295-2026-02-csec.xlsx` | Use this column to trace any row back to its exact source workbook. |
| `source_sheet` | string | Worksheet name within the source workbook. | N/A | `CSEC_02-2026` | Combine with source file, row, and column for exact cell provenance. |
...
| `category_path` | string | Hierarchy-aware path from table/section headings to the leaf category. | N/A | `Child Support Enforcement and Child S...` | Use this field instead of ``category`` when labels repeat in different subprograms. |
...
|---|---|---|---|---|
| Child Support Enforcement and Child Support Collections / Child Support Enforcement Program / Child Support Administrationa | `a` | Amounts include federal payments to states, U.S. territories, and Indian tribes. | Child Support Administrationa | `51295-2026-02-csec.xlsx` / `CSEC_02-2026` / R15C1 |
| Child Support Enforcement and Child Support Collections / Child Support Enforcement Program / Sequestrationb | `b` | The Further Consolidated Appropriations Act, 2024, extended sequestration for mandatory spending through fiscal year 2032. | Sequestrationb | `51295-2026-02-csec.xlsx` / `CSEC_02-2026` / R19C1 |
| Child Support Enforcement and Child Support Collections / Child Support Collectionsc / TANF Collections Retained by the Federal Government | `c` | Collections are recorded in the federal budget as negative outlays. | Child Support Collectionsc | `51295-2026-02-csec.xlsx` / `CSEC_02-2026` / R22C1 |
